...
import { DEFAULT_PAGE, DEFAULT_PAGE_SIZE, SEARCH_DELAY_MS } from '../utils/constants.js';
import { parsePositiveInt } from '../utils/pagination.js';


const ConfirmModal = lazy(() => import('../components/common/ConfirmModel.jsx'));
...
const Products = () => {
  ...
  const visibleProducts = useMemo(() => {
    const notDeleted = products.filter((p) => !deletedSet.has(p.id));
    return sortProducts(notDeleted, sort);
  }, [products, sort, deletedSet]);
  ...
  const handleDeleteClick = useCallback((product) => setDeleteTarget(product), []);
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    try {
      ...
      setToast(`"${deleteTarget.title}" deleted (local only).`);
  ...
  return (
    <div>
      ...
      {!loading && !error && visibleProducts.length > 0 && (
        <>
          <ProductTable products={visibleProducts} onDelete={handleDeleteClick} />
